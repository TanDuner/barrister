...
- (void)viewDidLoad {
    ...
    self.cameraController.sessionPreset = AVCaptureSessionPresetHigh;
    

    
    self.overlayView.session = self.cameraController.captureSession;
    self.overlayView.tapToFocusEnabled = NO;
...
-(void)didEndRecNumber:(NSString*)str image:(UIImage*)img
{
    ...
    NSString *cardStr = [str stringByReplacingOccurrencesOfString:@" " withString:@""];
    __weak typeof(*&self) weakSelf = self;
    [XuUItlity showLoading:@"识别中"];
    [self.proxy getCardInfoWithCardNum:cardStr WithBlock:^(id returnData, BOOL success) {
        [XuUItlity hideLoading];
        if (success) {
            if (weakSelf.cardBlock) {
                NSDictionary *dict = (NSDictionary *)returnData;
                
                BankCardModel *model = [[BankCardModel alloc] initWithDictionary:dict];
                model.cardNum = cardStr;
                weakSelf.cardBlock(model);
            }
            
        }
        else{
//            [XuUItlity showFailedHint:@"识别失败" completionBlock:nil];
        }
