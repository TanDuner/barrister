...
-(void)createView
{
    ...
    _areaTextField.keyboardType = UIKeyboardTypeDefault;
...
-(void)publishCaseSource
{
    ...
    [params setObject:_areaTextField.text forKey:@"area"];
    [params setObject:_emailTextField.text forKey:@"email"];
    [params setObject:_introTextView.text forKey:@"caseInfo"];
    ...
    [self.proxy publishCaseSourceWithParams:params block:^(id returnData, BOOL success) {
        if (success) {
            [XuUItlity showSucceedHint:@"发布成功" completionBlock:nil];
        }
        else
        {
